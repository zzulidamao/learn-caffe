...
void convert_dataset(const char* image_filename, const char* label_filename,
  ...
  // Open files
  std::ifstream image_file(image_filename, std::ios::in | std::ios::binary);//以二进制形式打开文件
  std::ifstream label_file(label_filename, std::ios::in | std::ios::binary);
  CHECK(image_file) << "Unable to open file " << image_filename;//判断是否打开
  ...
  image_file.read(reinterpret_cast<char*>(&magic), 4);//强制转为char*
...
int main(int argc, char** argv) {
  ...
  FLAGS_alsologtostderr = 1;//日志输出到窗口
