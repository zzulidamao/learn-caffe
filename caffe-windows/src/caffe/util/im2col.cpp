...
void im2col_cpu(const Dtype* data_im, const int channels,
  ...
  const int channel_size = height * width;//每个通道的像素数量
  for (int channel = channels; channel--; data_im += channel_size) {//从最高层通道开始，每次步长为一个通道的图像像素数量
    for (int kernel_row = 0; kernel_row < kernel_h; kernel_row++) {//卷积核高
      for (int kernel_col = 0; kernel_col < kernel_w; kernel_col++) {//卷积核宽
